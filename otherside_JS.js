//JavaScipt Document
function scrollToTop() { //smooth animation when scroll-to-top banner is clicked
    $("html, body").animate({
        scrollTop: 0
    }, 1000);
}
function disableScroll() { //disables scroll while animations are rendering, now decrepit
...
var backgroundChange = 1; //global variable to check whether a background change has occurred
$(document).ready(function() {
    $(window).scroll(function() { //runs repeated commands on scroll perpetually, may be optimized in future updates
        $('#disclaimer').css('visibility', 'hidden'); //hides disclaimer permanently on single scroll
        $(".onload_title").css("opacity", 1 - $(document).scrollTop() / 777);
        function dimming(target) { //a dimming function that fades out elements as they reach the top of the viewport
        ...
        var distanceFromTop = $(document).scrollTop(); //initializing local variable to check an element's distance from the document's top regardless of viewport positioning
        ...
        var targetHeight = $('.onload_title').height();//most of the document from here on out involves initializing and rendering animations to show embedded containers based on the viewport's height; further updates to this site may include an automated system that cuts down on processing requests
        ...
        if (distanceFromTop >= .935 * targetHeight) {
        ...
        if (distanceFromTop >= 1 * targetHeight) { //changing background color and text color from part 1 to part 2
                        ...
                        if (distanceFromTop >= 1.16 * targetHeight) {
                            ...
                            if (distanceFromTop >= 1.09 * targetHeight) {
                                ...
                                if (distanceFromTop >= 1.07 * targetHeight) {
                                    ...
                                    targetHeight += $('#slide12').height();
                                    if (distanceFromTop >= 1.035 * targetHeight) {
                                        $('#slide13').addClass('appear');
        ...
        dimming('#slide2_title'); //dimming the opacity of content relative to the viewport's position
